...
export const registerGetJobJclTool: ToolRegistrar = (server, ctx) => {
    server.tool('get_job_jcl', 'Retrieve the JCL that was submitted for a job (from JESJCLIN / JESJCL / JCL spool DDs). Supports custom line limit.', {
    ...
    }, securedHandler(ctx, 'get_job_jcl', async ({ jobId, maxLines }) => {
        ...
        const jclFile = files
            ...
            .sort((a, b) => JCL_DDS.indexOf(a.ddName.toUpperCase()) - JCL_DDS.indexOf(b.ddName.toUpperCase()))[0];
        if (!jclFile) {
        ...
        ctx.logger.debug({ jobId: normalizedId, ddName: jclFile.ddName, totalLines }, 'get_job_jcl');
        const header = formatContentHeader(`JCL — ${job.jobName} (${job.jobId}) DD=${jclFile.ddName}  [${totalLines} lines]`, truncated, totalLines, cap);
