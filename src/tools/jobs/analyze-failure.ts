...
const inputShape = {
    ...
    spoolDd: z
        .string()
        .optional()
        .describe('Force analysis to read a specific DD instead of the auto-priority list (e.g. "JESYSMSG").'),
};
export const registerAnalyzeFailureTool: ToolRegistrar = (server, ctx) => {
    server.tool('analyze_job_failure', 'Analyze a failed job: identify the abend code, failing/cancelled step, program, and a suggested fix. Produces a rich 3-section report.', inputShape, securedHandler(ctx, 'analyze_job_failure', async ({ jobId, includeEvidence, spoolDd }) => {
        ...
        ctx.logger.debug({ jobId: normalizedId, abendCode: analysis.abendCode, cancelledStep: analysis.cancelledStep, spoolDd: normalizedDd }, 'analyze_job_failure');
        ...
        if (!isFailedJob(analysis.job) && !analysis.abendCode && !analysis.cancelledStep) {
            return textResult(`Job ${analysis.job.jobName} (${analysis.job.jobId}) completed normally with return code ${analysis.job.returnCode ?? '(none)'}.\n\nNo failure analysis is required.`);
        }
        return textResult(formatAnalysisFull(analysis, { includeEvidence: includeEvidence !== false }));
    }));
};
