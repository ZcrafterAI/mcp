...
const inputShape = {
    hours: z
        ...
        .optional()
        .describe(
            'Historical look-back window in hours. Defaults to 168 (7 days), max 720 (30 days).',
        ),
    minFailures: z
        .number()
        .int()
        ...
        .optional()
        .describe(
            'Minimum number of failures a job must have in the window to appear in the report. Defaults to 1.',
        ),
};

export const predictBatchFailuresTool = defineTool({
    name: 'predict_batch_failures',
    description:
    ...
    async run({ hours, minFailures }, ctx) {
        ...
        const riskTable = renderTable(
            ['Job Name', 'Risk', 'Score', 'Failures', 'Top Abend', 'Abend Description', 'Trend'],
            risks.map((risk) => [
                risk.jobName,
                risk.riskLevel,
        ...
        ctx.logger.debug(
            { window, jobs: risks.length, critical: critical.length, high: high.length },
            'predict_batch_failures',
        );
        return textResult(
            formatStructuredResponse('Predictive Batch Failure Analysis', [
                ...
                { heading: 'Risk ranking', body: riskTable },
                {
                    ...
                    body:
                        recommendations ||
                        'No high-risk or critical jobs require immediate action.',
                },
