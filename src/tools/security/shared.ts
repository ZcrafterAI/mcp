...
import type { RacfAuditEntry } from '../../utils/racf-parser.js';
import { filterAuditEntries, parseRacfAuditLine } from '../../utils/racf-parser.js';
import { requireRacfAuditSource } from '../../zowe/rest-client.js';
...
export async function loadRacfAuditRecords(ctx: ToolContext): Promise<RacfAuditEntry[]> {
    ...
    return capped
        .split(/\r?\n/)
        .filter(Boolean)
        .map(parseRacfAuditLine);
}
/** Query RACF audit records with optional filters and result cap. */
export async function queryRacfAudit(ctx: ToolContext, filters: {
    user?: string;
    resource?: string;
    hours?: number;
    event?: string;
    result?: string;
    class?: string;
}, maxResults?: number): Promise<RacfAuditEntry[]> {
