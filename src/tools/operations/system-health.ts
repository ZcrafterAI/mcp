...
const inputShape = {
    lookbackHours: z
        ...
        .max(720)
        .optional()
        .describe('Scope the failed-job analysis to jobs that ended within this many hours. ' +
...
export const registerSystemHealthTool: ToolRegistrar = (server, ctx) => {
    server.tool('system_health_summary', 'Produce a high-level operational health snapshot: overall status (HEALTHY/DEGRADED/CRITICAL), active and queued jobs, recent failure count, top abend codes with descriptions, and per-owner failure breakdown.', inputShape, securedHandler(ctx, 'system_health_summary', async ({ lookbackHours }) => {
        ...
        const abendBody = topAbends.length === 0
            ? 'No failed jobs with recognizable abend codes.'
            : renderTable(['Abend Code', 'Count', 'Description', 'Affected Jobs'], topAbends.map((entry) => {
                const info = lookupAbend(entry.code);
                const jobList = entry.jobs.slice(0, 3).join(', ') + (entry.jobs.length > 3 ? ` +${entry.jobs.length - 3} more` : '');
                return [entry.code, String(entry.count), info?.title ?? '(no reference entry)', jobList];
            }));
        ...
        const ownerBody = ownerRows.length === 0
        ...
        return textResult(formatStructuredResponse('System Health Summary', [
