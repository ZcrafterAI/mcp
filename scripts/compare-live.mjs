...
function secretValues() {
  return [process.env.ZOSMF_PASSWORD, process.env.ZOSMF_TOKEN]
...
async function timedCall(server, name, args) {
  ...
  try {
    ...
    return { ok: false, ms: performance.now() - started, error: redact(error?.message ?? error) };
  }
}
...
function datasetNames(text) {
  return [...new Set((text.toUpperCase().match(/\b[A-Z$#@][A-Z0-9$#@-]{0,7}(?:\.[A-Z$#@][A-Z0-9$#@-]{0,7})+\b/g) ?? []))]
    .sort();
}
...
async function main() {
  ...
  try {
    ...
    for (const server of servers) {
      ...
      if (missing.length) throw new Error(`${server.label} is missing tools: ${missing.join(", ")}`);
    }

    const verify = {};
    for (const server of servers) verify[server.label] = await timedCall(server, "verify_zosmf_connection", {});
    const invalid = {};
    for (const server of servers) invalid[server.label] = await timedCall(server, "list_datasets", {});

    const results = [];
    for (const test of TOOL_CASES) {
      const samples = await pairedCalls(servers, test.name, test.args(options), options.runs, options.warmups);
      results.push({
        name: test.name,
        samples,
    ...
    printTable([
      ...
      ["Both connect", verifyPass ? "PASS" : "FAIL", verifyPass ? "z/OSMF reachable" : "connection failure"],
      ...results.map((result) => [result.name, result.correctness.pass ? "PASS" : "FAIL", result.correctness.note]),
      ["Invalid input", invalidPass ? "PASS" : "FAIL", invalidPass ? "both rejected it" : "one accepted bad input"],
    ]);

    console.log("\nPerformance");
    printTable([
      ["Operation", "Baseline median", "Candidate median", "Change", "Candidate p95", "Failures"],
      ...results.map((result) => [
        result.name,
    ...
    const correctnessPass = verifyPass && invalidPass && results.every((result) => result.correctness.pass);
